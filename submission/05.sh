...
utxo_vout_1=$(echo $tx | jq -r '.vout.[0]')
utxo_txid_2=$(echo $tx | jq -r '.txid')
utxo_vout_2=$(echo $tx | jq -r '.vout.[1]')

recipient=2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP

rawtxhex=$(bitcoin-cli -named createrawtransaction inputs='''[ { "txid": "'$utxo_txid_1'", "vout": '$utxo_vout_1' }, { "txid": "'$utxo_txid_2'", "vout": '$utxo_vout_2' } ]''' outputs='''{ "'$recipient'": 0.2 }''')
echo $rawtxhex
